...
read -rp "你需要TAG标签吗(Y/N): " chrony_install
    ...
    [yY][eE][sS] | [yY])
        ...
        docker run --name nginx-mtproxy -d -e tag="$tag" -e secret="$secret" -e domain="$domain" -p 88:88 -p $port:$port ellermister/nginx-mtproxy:latest
        ;;
    *)
...
echo -e "正在安装nginx-mtproxy... "
docker run --name nginx-mtproxy -d -e secret="$secret" -e domain="$domain" -p 88:88 -p $port:$port ellermister/nginx-mtproxy:latest
        ;;
    esac
...
# echo -e "输入 docker logs nginx-mtproxy 获取链接信息"
    ...
    echo -e "TG认证地址：http://${public_ip}:88/add.php"
